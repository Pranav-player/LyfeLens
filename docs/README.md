...
Backend API Layer (Node.js / Express)
        ↓
ML Inference Pipeline (MoveNet + classification modules)
        ↓
Real-time Guidance Layer (AR overlays + AI assistance)
```
...
```bash
cd LyfeLensMobile/lyfelens-mobile
npm install
npx expo start
```
...
```bash
cd LyfeLensMobile/lyfelens-mobile/lyfeLens-backend
npm install
node server.js
```
